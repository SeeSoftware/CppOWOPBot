...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::OWOPLua()
{
	static bool inited = false;
	static LanguageDefinition langDef;
	if (!inited)
	{
		...
		static const char* const owopIdentifiers[] = {
			"hook","Add","Remove","Run","Vec2i","Vec2f","Distance","Length","Dot","Normalize","GetNormalized","ToVec2i","ToVec2f"
		};
		for (auto& k : owopIdentifiers)
		{
			Identifier id;
			id.mDeclaration = "OWOP-Lua function";
			langDef.mIdentifiers.insert(std::make_pair(std::string(k), id));
		}
