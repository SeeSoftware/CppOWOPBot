...
void ConsoleTool::UpdateAndDrawConsole(float dt)
{
	...
	for (int y = 0; y < mResolution.y; y++)
		for (int x = 0; x < mResolution.x; x++)
		{
			uint16_t attribute = mTextAttributeBuffer[x + y * mResolution.x];

			sf::RectangleShape bgRect(sf::Vector2f((float)mFontSpacing.x, (float)mFontSpacing.y));
			bgRect.setPosition(sf::Vector2f((float)(x * mFontSpacing.x), (float)(y * mFontSpacing.y)));
			bgRect.setFillColor(CONSOLE_COLORS[(attribute & 0x00F0) >> 4]);
			mConsoleTexture.draw(bgRect);
			...
			text.setFillColor(((attribute&0x000F) == 0) ? sf::Color(170, 170, 170) : CONSOLE_COLORS[attribute & 0x000F]);
			text.setFont(mFont);
			text.setCharacterSize(FONT_SIZE);
			text.setScale(sf::Vector2f(1.0f, 1.0f)); //to unsmooth pixel edges
...
void ConsoleTool::UpdateTextBufferFromCMD()
{
	...
	mTextAttributeBuffer.resize(size);
	ConsoleHelper::ReadConsoleAttributes(mTextAttributeBuffer.data(), size, { 0,(short)top });
